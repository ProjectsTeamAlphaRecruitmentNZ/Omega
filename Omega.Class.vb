...
'****************************************************************************
' Do not debug in this generated code.  Debug the original AE directly.
'****************************************************************************
#ExternalSource("C:\Program Files (x86)\StayinFront\CRM\Libraries\lbrAnalytics.ae",58)
Public AnalyticsViewClassDefs as String

Public Sub lbrAnalytics_Startup(hWnd as Long)
	Dim viewClassDefs as string = AtServer.BuildAnalyticsTabAvailabilityList()
	Aspect("Elk9.Aspect").Properties.Item("Analytics_ViewClassDefs") = viewClassDefs
End Sub

Public Function BuildAnalyticsTabAvailabilityList() as String
	Dim ht As New Hashtable
	For Each inst as AnalyticView In AnalyticViews.Scan("Not IsEmpty(ViewsClassDefs)", "ViewsClassDefs", 0, "")
		Dim value: value = inst.ViewsClassDefs.Value
		For Each str As String In value.Split(new String() {"|"}, StringSplitOptions.RemoveEmptyEntries)
			If Not ht.Contains(str) Then
				ht.Add(str, "")
			End If
		Next
	Next

	Dim list As StringBuilder = New StringBuilder()
	For Each value As String In ht.Keys
...
Public Sub RenameLinkedAnalyticView(oView as IAeClassInst, transaction As IAeTransaction)
	If IsLinkedAnalyticsView(oView) Then
		Dim analyticsView As AnalyticView = GetLinkedAnalyticView(oView)
		If Not analyticsView Is Nothing Then
			Try
				If analyticsView.CanModify Then
					If analyticsView.ViewBarID.IsNull Then
						analyticsView.ViewBarID.Value = oView
					End If
					If analyticsView.ViewBarID.Value.Key = oView.Key Then
						analyticsView.Title.Value = oView.Title.Value
						analyticsView.Save(transaction)
					End If
				End If
			Catch ex as Exception
				If Analytics.Data.Logging.ShouldLog(LogWhat.Warnings) Then _
					Analytics.Data.Logging.Out.WriteLine(String.Format("Exception: {0} thrown in RenameLinkedAnalyticView", ex.Message))
			End Try		
		End If
	End if
End Sub

Public Function IsLinkedAnalyticsView(oView as IAeClassInst) as Boolean
	Dim controlClass As String = oView.ControlClass.Value.ToString()
			
	' if this is a linked Analytics view....
	IsLinkedAnalyticsView = controlClass.StartsWith("#Analytics.UI.EntireUIControl") AndAlso _
		Not IsDBNull(oView.State.Value)
End Function

Public Function GetLinkedAnalyticView(oView as IAeClassInst) as IAeClassInst
	Dim analyticsView As AnalyticView = Nothing
	
	Try
		'parse the view state xml looking for <link key="...">
		Dim doc As XmlDocument = new XmlDocument()
		doc.LoadXml(oView.State.Value)
		Dim node As XmlElement = doc.DocumentElement.FirstChild
	
		If node.Name = "link" Then
			'extract the linked object key and look it up	
			Dim key As String = node.GetAttribute("key")
			analyticsView = Classes.Item("AnalyticView").GetInstance(key)
		End If
	Catch ex as Exception
		If Analytics.Data.Logging.ShouldLog(LogWhat.Warnings) Then _
			Analytics.Data.Logging.Out.WriteLine(String.Format("Exception: {0} thrown in GetLinkedAnalyticView", ex.Message))
	End Try		
	
	GetLinkedAnalyticView = analyticsView
End Function

Public Function GetAnalyticsConfiguration(Of T)(view as Analytics.Data.IView, name as String) as T
	Dim viewType As Type = GetType(Analytics.Data.CView)
	If (viewType Is Nothing) Then Return Nothing

	Dim method As MethodInfo = viewType.GetMethod("GetConfiguration")
	If (method Is Nothing) Then Return Nothing

	Dim genericMethod As MethodInfo = method.MakeGenericMethod(GetType(T))
	If (genericMethod Is Nothing) Then Return Nothing

	return genericMethod.Invoke(view, new Object() { name })
End Function

Public Sub SetAnalyticsConfiguration(Of T)(view as Analytics.Data.IView, name as String, value as T)
	Dim viewType As Type = GetType(Analytics.Data.CView)
	If (viewType Is Nothing) Then Return

	Dim method As MethodInfo = viewType.GetMethod("SetConfiguration")
	If (method Is Nothing) Then Return

	Dim genericMethod As MethodInfo = method.MakeGenericMethod(GetType(T))
	If (genericMethod Is Nothing) Then Return

	genericMethod.Invoke(view, new Object() { name, value })
End Sub

' This is a function intended for models to use from Workflow to save images of analytics views
'  as files.  E.g. this could be for use within static "report packs" form mobile devices.
'  this returns the filename for chaining purposes
Public Function Analytics_SaveViewToImage( _
	analyticView as IAeClassInst, filterToInstance as IAeClassInst, viewFilter as String, _
	outputFileName as String, nWidth as Integer, nHeight as Integer) as String

	If analyticView Is Nothing Then Throw New ApplicationException("No view specified")

	Dim view as IView = Analytics.Data.Utils.InternalAPI.GetIViewFromAnalyticsView(analyticView)
	If view Is Nothing Then Throw New ApplicationException("View failed to deserialize")
	
	Dim ctl as New Analytics.UI.GraphGridControl
	Dim img as System.Drawing.Image = Nothing

	Try
		'Apply filter if provided
		If Not String.IsNullOrEmpty(viewFilter) Then view.Filter = viewFilter
		
		'Filter for given instance
		If Not filterToInstance Is Nothing Then view.Internal.AeObjectToFilterTo = filterToInstance

		ctl.Name = "m_graphGridControl"
		ctl.Width = nWidth
		ctl.Height = nHeight 
	
		' View generation will fail if the 'maximum number of data elements to allow before a warning will be
		' shown to the user' limit is exceeded. The code below can be used to override the default limit.
		' Dim val as Integer = GetAnalyticsConfiguration(Of Integer)(view, "WarningRowCount")
		' If ( val < 10 ) Then SetAnalyticsConfiguration(Of Integer)(view, "WarningRowCount", 10)
		
		'forces the control to NOT cause a view refresh, which kills the whole
		'  syncronous loading that we are trying to do here.
		ctl.AutoQueryOnViewChange = False
		
		ctl.Progress.AllowVisible = False
		
		'force the data to syncronously load
		Dim vdata as IViewData = view.Data
		
		ctl.View = view
		ctl.BackColor = System.Drawing.Color.White
		
		'save the image
		Dim iGraph as Analytics.UI.IGraphControl = ctl
		img = iGraph.Image
		
		Dim targetImageFormat as System.Drawing.Imaging.ImageFormat
		
		Select Case System.IO.Path.GetExtension(outputFileName).ToUpper()
			Case ".JPG"
				targetImageFormat = System.Drawing.Imaging.ImageFormat.Jpeg
			Case ".BMP"
				targetImageFormat = System.Drawing.Imaging.ImageFormat.Bmp
			Case ".PNG"
				targetImageFormat = System.Drawing.Imaging.ImageFormat.Png
			Case Else
				targetImageFormat = System.Drawing.Imaging.ImageFormat.Bmp ' default to Bitmap
		End Select
		
		img.Save(outputFileName, targetImageFormat)
	Finally
...
#End ExternalSource
	...
	End Function
	<AutoGenerated> _
	Public Function [Budgets]() As IAeFolder
		[Budgets]=Folders("Budgets")
	End Function
	<AutoGenerated> _
	...
	End Function
	<AutoGenerated> _
	Public Function [CandidateQualification]() As IAeFolder
		[CandidateQualification]=Folders("CandidateQualification")
	End Function
	<AutoGenerated> _
	Public Function [PublicHolidays]() As IAeFolder
		[PublicHolidays]=Folders("PublicHolidays")
	End Function
	<AutoGenerated> _
	...
	End Function
	<AutoGenerated> _
	Public Function [AnalyticViews]() As IAeFolder
		[AnalyticViews]=Folders("AnalyticViews")
	End Function
	<AutoGenerated> _
	Public Function [TopLevelAnalyticsViews]() As IAeFolder
		[TopLevelAnalyticsViews]=Folders("TopLevelAnalyticsViews")
	End Function
	<AutoGenerated> _
	Public Function [AnalyticViewGroups]() As IAeFolder
		[AnalyticViewGroups]=Folders("AnalyticViewGroups")
	End Function
	<AutoGenerated> _
	Public Function [AnalyticViewLinks]() As IAeFolder
		[AnalyticViewLinks]=Folders("AnalyticViewLinks")
	End Function
	<AutoGenerated> _
	Public Function [Requirements]() As IAeFolder
		[Requirements]=Folders("Requirements")
	End Function
	#End Region
